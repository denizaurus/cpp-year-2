...
{
	class No {};
	class Yes { No no[2]; }; // Разных размеров !

	static Yes test(B*);
	static No test(...); // произв. количество аргументов неизвестных типов

public:

	// Разрешение перегрузки выполняется во время компиляции
	// Определение занчения sizeof выполняется во время компиляции
	// Значение в enum вычисялются во время компиляции
